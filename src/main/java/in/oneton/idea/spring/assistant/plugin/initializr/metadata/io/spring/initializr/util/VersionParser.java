...
public class VersionParser {
    ...
    public Version parse(String text) {
        ...
        if (!matcher.matches()) {
            throw new InvalidVersionException("Could not determine version based on '" + text + "': version format " + "is Minor.Major.Patch.Qualifier " + "(e.g. 1.0.5.RELEASE)");
        }
        ...
        if ("x".equals(minor) || "x".equals(patch)) {
            ...
            if (latest == null) {
                return new Version(major, ("x".equals(minor) ? 999 : Integer.parseInt(minor)), ("x".equals(patch) ?
                        999 : Integer.parseInt(patch)), qualifier);
            }
