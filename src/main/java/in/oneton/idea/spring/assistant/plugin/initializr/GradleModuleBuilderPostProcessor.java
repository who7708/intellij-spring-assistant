...
public class GradleModuleBuilderPostProcessor implements ModuleBuilderPostProcessor {
    ...
    public boolean postProcess(Module module) {
        // TODO: Find a way to use GradleModuleBuilder instead of GradleProjectImportBuilder when adding a child
        ...
        if (gradleFile == null) { // not a gradle project
            ...
            AddModuleWizard addModuleWizard = new AddModuleWizard(project, gradleFile.getPath(), importProvider);
            if (addModuleWizard.getStepCount() > 0 && !addModuleWizard.showAndGet()) { // user has cancelled import
                // project prompt
                return true;
