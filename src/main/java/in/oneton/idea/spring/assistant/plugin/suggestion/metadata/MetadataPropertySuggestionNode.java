...
public class MetadataPropertySuggestionNode extends MetadataSuggestionNode {
    ...
    public static MetadataPropertySuggestionNode newInstance(String originalName,
        ...
        MetadataPropertySuggestionNode.MetadataPropertySuggestionNodeBuilder builder =
                MetadataPropertySuggestionNode.builder().name(sanitise(originalName)).originalName(originalName).property(property).parent(parent);
    ...
    public MetadataSuggestionNode findDeepestMetadataNode(String[] pathSegments, int pathSegmentStartIndex,
        ...
        if (haveMoreSegments) {
            ...
            // since this is the last node in the metadata subsection of the tree, lets just return if this is not
            // the last segment & last node
    ...
    public List<SuggestionNode> findDeepestSuggestionNode(Module module,
        ...
        if (haveMoreSegments) {
            if (!property.isLeaf(module)) {
                deepestMatch = property.findChildDeepestKeyMatch(module, matchesRootTillParentNode, pathSegments,
    ...
    public SortedSet<Suggestion> findKeySuggestionsForQueryPrefix(Module module, FileType fileType,
        ...
        if (!property.isDeprecatedError()) {
            ...
            if (lookingForConcreteNode) {
                return newSingleElementSortedSet(property.buildKeySuggestion(module, fileType, matchesRootTillMe,
                        numOfAncestors));
            } else {
                if (!property.isLeaf(module)) {
    ...
    public SortedSet<Suggestion> findValueSuggestionsForPrefix(Module module, FileType fileType,
                                                               ...
                                                               @Nullable Set<String> siblingsToExclude) {
        return property.findSuggestionsForValues(module, fileType, matchesRootTillMe, prefix, siblingsToExclude);
    }
    ...
    protected boolean hasOnlyOneChild(Module module) {
        // since we have to delegate any further lookups to the delegate (which has additional cost associated with
        // parsing & building childrenTrie dynamically)
        // lets always lie to caller that we have more than one child so that the search terminates at this node on
        // the initial lookup
        return false;
    }
