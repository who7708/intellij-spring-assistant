...
public class PsiCustomUtil {
    ...
    private static final Key<CachedValue<Map<String, GenericClassMemberWrapper>>> SPRING_ASSISTANT_PLUGIN_PROPERTY_TO_CLASS_MEMBER_WRAPPER_KEY = create("spring_assistant_plugin_property_to_class_member_wrapper");
    private static final Key<CachedValue<PsiType>> SPRING_ASSISTANT_PLUGIN_ERASE_FREE_TYPE_PARAMETER_TYPE_KEY =
            create("spring_assistant_plugin_eraseFreeTypeParameterType");
    private static final Key<CachedValue<PsiType>> SPRING_ASSISTANT_PLUGIN_FIRST_PARAMETER_TYPE_KEY = create(
            "spring_assistant_plugin_firstParameterType");
    private static final Key<CachedValue<PsiType>> SPRING_ASSISTANT_PLUGIN_RETURN_TYPE_KEY = create(
            "spring_assistant_plugin_returnType");
    ...
    public static PsiType safeGetValidType(@NotNull Module module, @NotNull String fqn) {
        try {
            ...
            PsiType type =
                    JavaPsiFacade.getInstance(module.getProject()).getParserFacade().createTypeFromText(fqn.replaceAll("\\$", "."), null);
    ...
    public static PsiType getReferredPsiType(PsiElement psiElement) {
        ...
        throw new RuntimeException("Method supports psiElement of type PsiField, PsiMethod & PsiClass only");
    }
            ...
            if (classResolveResult.isValidResult()) {
                ...
                if (psiClass.isEnum()) {
                    ...
                    return DOUBLE;
                        ...
                        // charset is a string
                        || "org.springframework.http.MediaType".equals(psiClass.getQualifiedName()) || ("java.net" +
                        ".InetAddress").equals(psiClass.getQualifiedName()) // ip address or hostname
                        ...
                        || "java.util.Date".equals(psiClass.getQualifiedName()) || "java.sql.Date".equals(psiClass.getQualifiedName()) || "java.time.LocalDate".equals(psiClass.getQualifiedName()) || "java.time.LocalTime".equals(psiClass.getQualifiedName()) || "java.time.LocalDateTime".equals(psiClass.getQualifiedName()) || "java.time.ZoneId".equals(psiClass.getQualifiedName()) || "java.time.ZonedDateTime".equals(psiClass.getQualifiedName()) || "java.time.Instant".equals(psiClass.getQualifiedName()) || "java.time.Duration".equals(psiClass.getQualifiedName())// date & duation
                        || "java.time.Period".equals(psiClass.getQualifiedName()) || "java.util.Locale".equals(psiClass.getQualifiedName())) {
                    return STRING;
    ...
    private static SuggestionNodeType getSuggestionNodeTypeForPrimitive(PsiType type) {
        if (PsiType.BOOLEAN.equals(type) || PsiType.BOOLEAN.equals(PsiPrimitiveType.getUnboxedType(type))) {
            return BOOLEAN;
        } else if (PsiType.BYTE.equals(type) || PsiType.BYTE.equals(PsiPrimitiveType.getUnboxedType(type))) {
            return BYTE;
        } else if (PsiType.SHORT.equals(type) || PsiType.SHORT.equals(PsiPrimitiveType.getUnboxedType(type))) {
            return SHORT;
        } else if (PsiType.INT.equals(type) || PsiType.INT.equals(PsiPrimitiveType.getUnboxedType(type))) {
            return INT;
        } else if (PsiType.LONG.equals(type) || PsiType.LONG.equals(PsiPrimitiveType.getUnboxedType(type))) {
            return LONG;
        } else if (PsiType.FLOAT.equals(type) || PsiType.FLOAT.equals(PsiPrimitiveType.getUnboxedType(type))) {
            return FLOAT;
        } else if (PsiType.DOUBLE.equals(type) || PsiType.DOUBLE.equals(PsiPrimitiveType.getUnboxedType(type))) {
            return DOUBLE;
        } else if (PsiType.CHAR.equals(type) || PsiType.CHAR.equals(PsiPrimitiveType.getUnboxedType(type))) {
            return CHAR;
        }
        return null;
    }
    ...
    public static String toClassNonQualifiedName(@NotNull PsiType type) {
        if (type instanceof PsiArrayType) {
            String componentLongName = toClassNonQualifiedName(((PsiArrayType) type).getComponentType());
            if (componentLongName != null) {
                return componentLongName + "[]";
            }
        } else if (type instanceof PsiPrimitiveType) {
    ...
    private static boolean isMap(@NotNull PsiClass psiClass) {
        return isClassSameOrDescendantOf(psiClass, JAVA_UTIL_MAP) || isClassSameOrDescendantOf(psiClass, "java.util" +
                ".Hashtable");
    }
    ...
    private static boolean isClassSameOrDescendantOf(@NotNull PsiClass psiClass, String expectedClassFqn) {
        return psiClass.getQualifiedName() != null && isInheritor(psiClass, expectedClassFqn);
    }

    @NotNull
    public static PsiType getBoxedTypeFromPrimitiveType(Module module, PsiPrimitiveType primitiveType) {
    ...
    public static Set<PsiClass> computeDependencies(Module module, @NotNull PsiType type) {
        ...
        if (isValidType(type)) {
            ...
            if (type instanceof PsiClassType) {
                ...
                TObjectHashingStrategy<PsiClass> nameComparingHashingStrategy = new TObjectHashingStrategy<PsiClass>() {
                    @Override
                ...
                for (PsiType typeParam : typeParams) {
                    if (typeParam != null) { // if the user specified raw class such as Map instead of Map<String,
                        // String>
            ...
            throw new IllegalAccessError("Only supports PsiArrayType, PsiPrimitiveType, PsiWildcardType, " +
                    "PsiCapturedWildcardType & PsiClassType. Does not support type: " + originalType.getCanonicalText());
        }
        return null;
    }
    ...
    public static boolean isValidType(@NotNull PsiType type) {
        if (!type.isValid()) {
            TimeoutUtil.sleep(1); // to see if processing in another thread suddenly makes the type valid again
            // (which is a bug)
            if (!type.isValid()) {
                return false;
            }
    ...
    private static boolean isValidElement(@NotNull PsiElement element) {
        if (!element.isValid()) {
            TimeoutUtil.sleep(1); // to see if processing in another thread suddenly makes the element valid again
            // (which is a bug)
    ...
    @Nullable
    public static Map<String, GenericClassMemberWrapper> getSanitisedPropertyToPsiMemberWrapper(@Nullable PsiClass psiClass) {
        if (psiClass != null) {
            ...
            if (isSimplePropertyGetter(method)) {
                ...
                if (acceptableMember != null) {
                    memberNameToMemberWrapper.put(sanitise(propertyName),
                            new GenericClassMemberWrapper(acceptableMember));
                }
            }
        }
    ...
    private static PsiType eraseFreeTypeParameters(@Nullable PsiType psiType, @Nullable PsiClass containingClass) {
        ...
        return getElementFactory(containingClass.getProject()).createRawSubstitutor(containingClass).substitute(psiType);
    }
    ...
    @Nullable
    private static PsiMethod findInstancePropertySetter(@NotNull PsiClass psiClass, @Nullable String propertyName) {
        ...
        for (PsiMethod setter : setters) {
            if (setter.hasModifierProperty(PUBLIC) && !setter.hasModifierProperty(STATIC) && isSimplePropertySetter(setter)) {
                return setter;
            }
        }
        return null;
    ...
    public static String computeDocumentation(PsiMember member) {
        ...
        if (docComment != null) {
            ...
            new JavaDocInfoGenerator(member.getProject(), member).generateCommonSection(builder, docComment);
