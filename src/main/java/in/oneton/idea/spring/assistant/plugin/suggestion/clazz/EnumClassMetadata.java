...
public class EnumClassMetadata extends ClassMetadata {
    ...
    protected Collection<? extends SuggestionDocumentationHelper> doFindDirectChildrenForQueryPrefix(Module module,
        ...
        if (childrenTrie != null && childLookup != null) {
            ...
            if (!isEmpty(prefixMap)) {
                return getMatchStreamAfterExclusions(childLookup, prefixMap.values(), siblingsToExclude).map(EnumKeySuggestionDocumentationHelper::new).collect(toList());
            }
        }
        return null;
    }
    ...
    private Stream<PsiField> getMatchStreamAfterExclusions(@NotNull Map<String, PsiField> childLookup,
                                                           Collection<PsiField> values,
                                                           @Nullable Set<String> siblingsToExclude) {
        if (siblingsToExclude != null) {
            Set<PsiField> exclusionMembers = siblingsToExclude.stream().map(childLookup::get).collect(toSet());
    ...
    protected List<SuggestionNode> doFindDeepestSuggestionNode(Module module,
        ...
        throw new IllegalAccessError("Should not be called. To use as a map key call findDirectChild(..) instead");
    }
    ...
    protected SortedSet<Suggestion> doFindKeySuggestionsForQueryPrefix(Module module, FileType fileType,
        ...
        throw new IllegalAccessError("Should not be called. To use as a map key call " +
                "doFindDirectChildrenForQueryPrefix(..) instead");
    }
    ...
    protected SortedSet<Suggestion> doFindValueSuggestionsForPrefix(Module module, FileType fileType,
        ...
        if (childrenTrie != null && childLookup != null) {
            ...
            if (!isEmpty(prefixMap)) {
                return getMatchStreamAfterExclusions(childLookup, prefixMap.values(), siblingsToExclude).map(psiField -> newSuggestion(fileType, matchesRootTillMe, matchesRootTillMe.size(), true, psiField)).collect(toCollection(TreeSet::new));
            }
        }
        return null;
    }
    ...
    protected String doGetDocumentationForValue(Module module, String nodeNavigationPathDotDelimited,
        ...
        if (childLookup != null) {
            ...
            return "<b>" + nodeNavigationPathDotDelimited + "</b> = <b>" + unescapeValue(originalValue) + "</b>" + new JavaDocumentationProvider().generateDoc(type, type);
        }
        return null;
    }
    ...
    private Suggestion newSuggestion(FileType fileType, List<SuggestionNode> matchesRootTillMe, int numOfAncestors,
                                     boolean forValue, @NotNull PsiField value) {
        Suggestion.SuggestionBuilder builder =
                Suggestion.builder().numOfAncestors(numOfAncestors).matchesTopFirst(matchesRootTillMe).shortType(toClassNonQualifiedName(type)).description(computeDocumentation(value)).icon(ENUM.getIcon()).fileType(fileType);
        if (forValue) {
    ...
    class EnumKeySuggestionDocumentationHelper implements SuggestionDocumentationHelper {
        ...
        public String getDocumentationForKey(Module module, String nodeNavigationPathDotDelimited) {
            return "<b>" + nodeNavigationPathDotDelimited + "</b>" + new JavaDocumentationProvider().generateDoc(field, field);
        }
