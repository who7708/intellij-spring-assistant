...
public class ProjectDetailsStep extends ModuleWizardStep implements Disposable {
            ...
            getApplication().executeOnPooledThread(() -> {
                try {
                    InitializerMetadata metadata =
                            request(request.getServerUrl()).accept(CONTENT_TYPE).userAgent(userAgent()).connect(httpRequest -> {
                        invokeLater(() -> loadingPanel.setLoadingText("Please wait.. Spring Initializr options are " +
                                "being loaded.."));
                        ...
                        builder.registerTypeAdapter(DEPENDENCY_LINKS_LIST_TYPE, new DependencyOneOrMoreDeserializer());
                    ...
                    invokeLater(() -> showErrorDialog("Error while fetching metadata from server '" + request.getServerUrl() + "'\nPlease check URL, network and proxy settings.\n\nError message:\n" + e.getMessage(), "Fetch Error"));
                } catch (JsonSyntaxException | JsonIOException e) {
                    invokeLater(() -> showErrorDialog("Error while parsing metadata from server '" + request.getServerUrl() + "'\nMake sure you connected to the correct initializr server." + "\n\nError message:\n" + e.getMessage(), "Metadata Error"));
                } finally {
                    invokeLater(() -> {
    ...
    public boolean validate() throws ConfigurationException {
        if (request.getMetadata() == null) {
            if (fetchInProgress) {
                throw new ConfigurationException("Metadata is being fetch from server. Please wait", "Please wait");
            } else {
                throw new ConfigurationException("Could not fetch metadata from server. Please go back & check server" +
                        " details", "Fetch Error");
            }
        }
