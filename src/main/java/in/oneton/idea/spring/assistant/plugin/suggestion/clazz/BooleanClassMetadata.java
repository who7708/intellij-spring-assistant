...
public class BooleanClassMetadata extends ClassMetadata {
    ...
    protected SortedSet<Suggestion> doFindValueSuggestionsForPrefix(Module module, FileType fileType,
        ...
        if (!isEmpty(matchesMap)) {
            ...
            return matchStream.map(val -> newSuggestion(fileType, matchesRootTillMe, matchesRootTillMe.size(), true,
                    val)).collect(toCollection(TreeSet::new));
        }
        return null;
    }
    ...
    private Stream<Boolean> getMatchStreamAfterExclusion(SortedMap<String, Boolean> prefixMap,
        ...
        if (siblingsToExclude != null) {
            matchStream = prefixMap.values().stream().filter(value -> !siblingsToExclude.contains(value.toString()));
        } else {
    ...
    private Suggestion newSuggestion(FileType fileType, List<SuggestionNode> matchesRootTillMe, int numOfAncestors,
                                     boolean forValue, boolean value) {
        Suggestion.SuggestionBuilder builder =
                Suggestion.builder().numOfAncestors(numOfAncestors).matchesTopFirst(matchesRootTillMe).shortType(
                        "Boolean").icon(ENUM.getIcon()).fileType(fileType);
        if (forValue) {
    ...
    private class BooleanKeySuggestionDocumentationHelper implements SuggestionDocumentationHelper {
        ...
        public String getDocumentationForKey(Module module, String nodeNavigationPathDotDelimited) {
             ...
             * <p>Long description</p>
             */
            StringBuilder builder = new StringBuilder().append("<b>").append(nodeNavigationPathDotDelimited).append(
                    "</b>");
