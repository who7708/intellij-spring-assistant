...
public class DependencySelection implements Disposable, DependencySelectionChangeListener {
    ...
    @NotNull
    private final Map<DependencyGroup, List<Dependency>> filteredGroupToDependencies = new THashMap<>();
    ...
    public void init(ProjectCreationRequest request) {
        ...
        AnAction findFocusAction = new AnAction() {
            ...
            public void actionPerformed(AnActionEvent e) {
                getGlobalInstance().doWhenFocusSettlesDown(() -> getGlobalInstance().requestFocus(filter.getTextEditor(), true));
            }
        };
        ...
        Version defaultBootVersion = request.getSetVersion(bootVersions, bootVersionComposite.getDefaultValue());
        String defaultBootVersionId = defaultBootVersion != null ? defaultBootVersion.toString() : null;

        CollectionComboBoxModel<IdAndName> bootVersionComboBoxModel = newCollectionComboBoxModel(bootVersions,
                defaultBootVersionId);
        bootVersion.setModel(bootVersionComboBoxModel);

        ColoredListCellRenderer<DependencyGroup> categoryRenderer = new ColoredListCellRenderer<DependencyGroup>() {
        ...
        new ListSpeedSearch<>(groups,
                (Function<DependencyGroup, String>) value -> ((DependencyGroup) value).getName());
        ...
        dependencyNameAndDescriptionIndex =
                new AutocompleteEngine.Builder<DependencyGroupAndDependency>().setIndex(new DependencyGroupAndDependencyAdapter()).setAnalyzers(new LowerCaseTransformer(), new StopwordsAwareWordTokenizer()).build();
        ...
        perGroupDependencyTableModel = new PerGroupDependencyTableModel(perGroupDependencyTable, request,
            ...
            protected void textChanged(DocumentEvent e) {
                searchTextChangesThrottler.throttle(() -> {
                    ...
                    if (!isEmpty(searchText)) {
                        ...
                        for (DependencyGroupAndDependency groupAndDependency : groupAndDependencies) {
                            List<Dependency> dependencies =
                                    filteredGroupToDependencies.getOrDefault(groupAndDependency.getGroup(),
                                            new ArrayList<>());
                    ...
                    if (filteringGroupListModel.getSize() != 0) {
                        // if there is a deferred dependency that needs to be selected, lets select this both in the
                        // groups & also in the per group dependency table
                        if (deferreredDependecySelection != null) {
                            Optional<DependencyGroup> groupOptional =
                                    metadata.getDependencyComposite().findGroupForDependency(deferreredDependecySelection);
    ...
    private void rebuildFilteredGroupsAndDependencies(List<DependencyGroup> dependencyGroups) {
        dependencyGroups.forEach(group -> {
            List<Dependency> dependencies = filteredGroupToDependencies.getOrDefault(group, new ArrayList<>());
    ...
    public void onDependencySelected(@Nullable Dependency dependency) {
        if (dependency != null) {
            ...
            DependencyGroup dependencyGroup = groupOptional.get();
            int groupIndex = filteringGroupListModel.getElementIndex(dependencyGroup);
            if (groupIndex == -1) { // this can happen if the search string is set, lets clear search string in this
                // case
                filter.setText(null);
