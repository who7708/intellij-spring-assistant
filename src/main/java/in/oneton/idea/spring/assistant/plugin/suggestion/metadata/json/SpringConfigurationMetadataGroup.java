...
public class SpringConfigurationMetadataGroup {
    ...
    public String getDocumentation(String nodeNavigationPathDotDelimited) {
        ...
        if (sourceType != null) {
            ...
            if (!sourceTypeInJavadocFormat.equals(removeGenerics(className))) {
                ...
                DocumentationManager.createHyperlink(buffer,
                        methodForDocumentationNavigation(sourceTypeInJavadocFormat), sourceTypeInJavadocFormat, false);
    ...
    public Suggestion newSuggestion(FileType fileType, List<SuggestionNode> matchesRootTillMe, int numOfAncestors) {
        return Suggestion.builder().suggestionToDisplay(GenericUtil.dotDelimitedOriginalNames(matchesRootTillMe,
                numOfAncestors)).description(description).shortType(shortenedType(className)).numOfAncestors(numOfAncestors).matchesTopFirst(matchesRootTillMe).icon(nodeType.getIcon()).fileType(fileType).build();
    }
