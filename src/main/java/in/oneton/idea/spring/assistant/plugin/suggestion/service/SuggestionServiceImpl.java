...
public class SuggestionServiceImpl implements SuggestionService {
    ...
    private final Map<String, Map<String, MetadataContainerInfo>> moduleNameToSeenContainerPathToContainerInfo;
    /**
        ...
        currentExecution = getApplication().executeOnPooledThread(() -> {
            getApplication().runReadAction(() -> {
                ...
                try {
                    ...
                    // OrderEnumerator.orderEntries(project) is returning everything from all modules including root
                    // level module(which is called project in gradle terms)
    ...
    public List<SuggestionNode> findMatchedNodesRootTillEnd(Project project, Module module,
        ...
        if (moduleNameToRootSearchIndex.containsKey(module.getName())) {
            String[] pathSegments =
                    containerElements.stream().flatMap(element -> stream(toSanitizedPathSegments(element))).toArray(String[]::new);
    ...
    public boolean canProvideSuggestions(Project project, Module module) {
        Trie<String, MetadataSuggestionNode> rootSearchIndex = moduleNameToRootSearchIndex.get(module.getName());
        return rootSearchIndex != null && rootSearchIndex.size() != 0;
    }

    @Override
    public List<LookupElementBuilder> findSuggestionsForQueryPrefix(Project project, Module module, FileType fileType
            , PsiElement element, @Nullable List<String> ancestralKeys, String queryWithDotDelimitedPrefixes,
                                                                    @Nullable Set<String> siblingsToExclude) {
        return doFindSuggestionsForQueryPrefix(module, moduleNameToRootSearchIndex.get(module.getName()), fileType,
                element, ancestralKeys, queryWithDotDelimitedPrefixes, siblingsToExclude);
    }

    private List<MetadataContainerInfo> computeNewContainersToProcess(OrderEnumerator orderEnumerator, Map<String,
            MetadataContainerInfo> seenContainerPathToContainerInfo) {
        ...
        for (VirtualFile metadataFileContainer : orderEnumerator.recursively().classes().getRoots()) {
            ...
            for (MetadataContainerInfo metadataContainerInfo : metadataContainerInfos) {
                boolean seenBefore =
                        seenContainerPathToContainerInfo.containsKey(metadataContainerInfo.getContainerArchiveOrFileRef());

                boolean updatedSinceLastSeen = false;
                if (seenBefore) {
                    MetadataContainerInfo seenMetadataContainerInfo =
                            seenContainerPathToContainerInfo.get(metadataContainerInfo.getContainerArchiveOrFileRef());
                    updatedSinceLastSeen = metadataContainerInfo.isModified(seenMetadataContainerInfo);
                    if (updatedSinceLastSeen) {
                        debug(() -> log.debug("Container seems to have been updated. Previous version: " + seenMetadataContainerInfo + "; Newer version: " + metadataContainerInfo));
                    }
                }
    ...
    private List<LookupElementBuilder> doFindSuggestionsForQueryPrefix(Module module, Trie<String,
            MetadataSuggestionNode> rootSearchIndex, FileType fileType, PsiElement element,
        ...
        try {
            ...
            if (ancestralKeys != null) {
                String[] ancestralKeySegments =
                        ancestralKeys.stream().flatMap(key -> stream(toRawPathSegments(key))).toArray(String[]::new);
                ...
                if (rootNode != null) {
                    ...
                    if (ancestralKeySegments.length > 1) {
                        ...
                        matchesRootToDeepest = rootNode.findDeepestSuggestionNode(module, modifiableList(rootNode),
                    ...
                    if (startSearchFrom != null) {
                        // if search start node is a leaf, this means, the user is looking for values for the given
                        // key, lets find the suggestions for values
                ...
                if (siblingsToExclude != null) {
                    Set<MetadataSuggestionNode> nodesToExclude =
                            siblingsToExclude.stream().flatMap(exclude -> rootSearchIndex.prefixMap(exclude).values().stream()).collect(toSet());
    ...
    private Set<Suggestion> doFindSuggestionsForQueryPrefix(Module module, FileType fileType,
        ...
        for (MetadataSuggestionNode suggestionNode : nodesToSearchWithin) {
            Set<Suggestion> matchedSuggestions = suggestionNode.findKeySuggestionsForQueryPrefix(module, fileType,
                    modifiableList(suggestionNode), 0, querySegmentPrefixes, querySegmentPrefixStartIndex);
     ...
     */
    private List<MetadataContainerInfo> computeContainersToRemove(OrderEnumerator orderEnumerator, Map<String,
            MetadataContainerInfo> seenContainerPathToContainerInfo) {
        Set<String> newContainerPaths =
                stream(orderEnumerator.recursively().classes().getRoots()).flatMap(MetadataContainerInfo::getContainerArchiveOrFileRefs).collect(toSet());
    ...
    private void processContainers(Module module, List<MetadataContainerInfo> containersToProcess,
                                   ...
                                   Map<String, MetadataContainerInfo> seenContainerPathToContainerInfo, Trie<String,
            MetadataSuggestionNode> rootSearchIndex) {
        // Lets remove references to files that are no longer present in classpath
        containersToRemove.forEach(container -> removeReferences(seenContainerPathToContainerInfo, rootSearchIndex,
                container));

        for (MetadataContainerInfo metadataContainerInfo : containersToProcess) {
            // lets remove existing references from search index, as these files are modified, so that we can rebuild
            // index
            ...
            try (InputStream inputStream = metadataContainerInfo.getMetadataFile().getInputStream()) {
                ...
                SpringConfigurationMetadata springConfigurationMetadata =
                        gsonBuilder.create().fromJson(new BufferedReader(new InputStreamReader(inputStream)),
    ...
    private void reindexModule(List<MetadataContainerInfo> newProjectSourcesToProcess,
        ...
        Map<String, MetadataContainerInfo> moduleSeenContainerPathToSeenContainerInfo =
                moduleNameToSeenContainerPathToContainerInfo.computeIfAbsent(module.getName(), k -> new THashMap<>());

        Trie<String, MetadataSuggestionNode> moduleRootSearchIndex = moduleNameToRootSearchIndex.get(module.getName());
        ...
        List<MetadataContainerInfo> newModuleContainersToProcess =
                computeNewContainersToProcess(moduleOrderEnumerator, moduleSeenContainerPathToSeenContainerInfo);
    ...
    private void buildMetadataHierarchy(Module module, Trie<String, MetadataSuggestionNode> rootSearchIndex,
    ...
    private void addHintsToIndex(Module module, Trie<String, MetadataSuggestionNode> rootSearchIndex,
        ...
        if (hints != null) {
            ...
            for (SpringConfigurationMetadataHint hint : hints) {
                ...
                MetadataSuggestionNode closestMetadata = findDeepestMetadataMatch(rootSearchIndex, pathSegments, true);
                if (closestMetadata != null) {
                    if (!closestMetadata.isProperty()) {
                        log.warn("Unexpected hint " + hint.getName() + " is assigned to  group " + closestMetadata.getPathFromRoot(module) + " found. Hints can be only assigned to property. Ignoring the hint completely.Existing group belongs to (" + closestMetadata.getBelongsTo().stream().collect(joining(",")) + "), New hint belongs " + containerPath);
                    } else {
                        MetadataPropertySuggestionNode propertySuggestionNode = (MetadataPropertySuggestionNode) closestMetadata;
    ...
    private void addPropertiesToIndex(Module module, Trie<String, MetadataSuggestionNode> rootSearchIndex,
                                      SpringConfigurationMetadata springConfigurationMetadata,
        ...
        for (SpringConfigurationMetadataProperty property : properties) {
            ...
            if (closestMetadata == null) { // path does not have a corresponding root element
                ...
                if (onlyRootSegmentExists) {
                    closestMetadata = MetadataPropertySuggestionNode.newInstance(rawPathSegments[0], property, null,
                            containerArchiveOrFileRef);
                } else {
                    closestMetadata = MetadataNonPropertySuggestionNode.newInstance(rawPathSegments[0], null,
                            containerArchiveOrFileRef);
                }
            ...
            if (haveMoreSegmentsLeft) {
                if (!closestMetadata.isProperty()) {
                    ((MetadataNonPropertySuggestionNode) closestMetadata).addChildren(property,
                            rawPathSegments, startIndex, containerArchiveOrFileRef);
                } else {
                    log.warn("Detected conflict between a new group & existing property for suggestion path " + closestMetadata.getPathFromRoot(module) + ". Ignoring property. Existing non property node belongs to (" + closestMetadata.getBelongsTo().stream().collect(joining(",")) + "), New property belongs to " + containerArchiveOrFileRef);
                }
            } else {
                if (!closestMetadata.isProperty()) {
                    log.warn("Detected conflict between a new metadata property & existing non property node for " +
                            "suggestion path " + closestMetadata.getPathFromRoot(module) + ". Ignoring property. " +
                            "Existing non property node belongs to (" + closestMetadata.getBelongsTo().stream().collect(joining(",")) + "), New property belongs to " + containerArchiveOrFileRef);
                } else {
                    closestMetadata.addRefCascadeTillRoot(containerArchiveOrFileRef);
                    log.debug("Detected a duplicate metadata property for suggestion path " + closestMetadata.getPathFromRoot(module) + ". Ignoring property. Existing property belongs to (" + closestMetadata.getBelongsTo().stream().collect(joining(",")) + "), New property belongs to " + containerArchiveOrFileRef);
                }
            }
        }
    }
    ...
    private void addGroupsToIndex(Module module, Trie<String, MetadataSuggestionNode> rootSearchIndex,
        ...
        if (groups != null) {
            ...
            for (SpringConfigurationMetadataGroup group : groups) {
                ...
                int startIndex;
                if (closestMetadata == null) { // path does not have a corresponding root element
                    // lets build just the root element. Rest of the path segments will be taken care of by the
                    // addChildren method
                    boolean onlyRootSegmentExists = pathSegments.length == 1;
                    MetadataNonPropertySuggestionNode newGroupSuggestionNode =
                            MetadataNonPropertySuggestionNode.newInstance(rawPathSegments[0], null,
                                    containerArchiveOrFileRef);
                    if (onlyRootSegmentExists) {
                ...
                if (closestMetadata.isProperty()) {
                    log.warn("Detected conflict between an existing metadata property & new group for suggestion path" +
                            " " + closestMetadata.getPathFromRoot(module) + ". Ignoring new group. Existing Property " +
                            "belongs to (" + closestMetadata.getBelongsTo().stream().collect(joining(",")) + "), New " +
                            "Group belongs to " + containerArchiveOrFileRef);
                } else {
                    // lets add container as a reference till root
                    MetadataNonPropertySuggestionNode groupSuggestionNode = (MetadataNonPropertySuggestionNode) closestMetadata;
                    groupSuggestionNode.addRefCascadeTillRoot(containerArchiveOrFileRef);

                    boolean haveMoreSegmentsLeft = startIndex < rawPathSegments.length;
                    if (haveMoreSegmentsLeft) {
                        groupSuggestionNode.addChildren(module, group, rawPathSegments, startIndex,
                                containerArchiveOrFileRef);
                    } else {
                        // Node is an intermediate node that has neither group nor property assigned to it, lets
                        // assign this group to it
                        // Can happen when `a.b.c` is already added to the metadata tree from an earlier metadata
                        // source & now we are trying to add a group for `a.b`
                        // In this e.g, startIndex would be 2. So, there is no point in adding children. We only need
    ...
    private MetadataSuggestionNode findDeepestMetadataMatch(Map<String, MetadataSuggestionNode> roots,
        ...
        if (closestMatchedRoot != null) {
            closestMatchedRoot = closestMatchedRoot.findDeepestMetadataNode(pathSegments, 1, matchAllSegments);
        }
        return closestMatchedRoot;
    }

    private void removeReferences(Map<String, MetadataContainerInfo> containerPathToContainerInfo, Trie<String,
            MetadataSuggestionNode> rootSearchIndex, MetadataContainerInfo metadataContainerInfo) {
        ...
        while (searchIndexIterator.hasNext()) {
            ...
            if (root != null) {
                boolean removeTree = ((MetadataSuggestionNode) root).removeRefCascadeDown(metadataContainerInfo.getContainerArchiveOrFileRef());
                if (removeTree) {
     ...
     * For eg., to enable debug logging, go `Help > Debug log settings` & type `#in.oneton.idea.spring.assistant.plugin
     * .suggestion.service.SuggestionServiceImpl`
     *
