...
public class Suggestion implements Comparable<Suggestion> {
    ...
    private static final LookupElementRenderer<LookupElement> CUSTOM_SUGGESTION_RENDERER =
        ...
        public void renderElement(LookupElement element, LookupElementPresentation presentation) {
            ...
            if (suggestion.defaultValue != null) {
                ...
                TextAttributes attrs = EditorColorsManager.getInstance().getGlobalScheme().getAttributes(SCALAR_TEXT);
     ...
     * <ol>
     * <li>Storing the whole value (support dynamic nodes aswell, as a single PsiClass as leaf might be referred via
     * multiple paths)</li>
     * <li>Storing reference to leaf & navigate up till the root (efficient)</li>
     * </ol>
     * The second solution does not address suggestions that are derived from {@link ClassMetadata} as these nodes are
     * not tied to a single branch of the suggestion tree
     */
    @NotNull
    ...
    @Builder
    public Suggestion(@NotNull String suggestionToDisplay, @Nullable String description, @Nullable String shortType,
                      @Nullable String defaultValue,
                      @Nullable SpringConfigurationMetadataDeprecationLevel deprecationLevel, @NotNull List<?
            extends SuggestionNode> matchesTopFirst, int numOfAncestors, boolean forValue,
    ...
    public LookupElementBuilder newLookupElement() {
        ...
        if (forValue) {
            ...
            builder =
                    builder.withRenderer(CUSTOM_SUGGESTION_RENDERER).withInsertHandler(fileType.newKeyInsertHandler());
        }
        return builder;
    }
    ...
    public int compareTo(@NotNull Suggestion other) {
        int pathRootToLeafComparisonValue = pathDotDelimitedRootToLeaf.compareTo(other.pathDotDelimitedRootToLeaf);
