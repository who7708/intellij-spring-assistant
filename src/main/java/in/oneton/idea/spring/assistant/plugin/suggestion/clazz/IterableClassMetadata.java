...
public class IterableClassMetadata extends ClassMetadata {
    ...
    protected Collection<? extends SuggestionDocumentationHelper> doFindDirectChildrenForQueryPrefix(Module module,
        ...
        return doWithDelegateOrReturnNull(delegate -> delegate.findDirectChildrenForQueryPrefix(module,
    ...
    protected List<SuggestionNode> doFindDeepestSuggestionNode(Module module,
        ...
        return doWithDelegateAndReturn(delegate -> {
            ...
            SuggestionDocumentationHelper directChildKeyMatch = delegate.findDirectChild(module, pathSegment);
            if (directChildKeyMatch != null) {
                // TODO: Need to identify a better mechanism than this dirty way. Probably use ClassSuggestionNode as
                //  return type for findDirectChildrenForQueryPrefix
                // since we are in an iterable(multiple values), keys would be requested, only if the object we are
                // referring is not a leaf => GenericClassWrapper
                ...
                if (lastPathSegment) {
                    ...
                    return wrappedNode.findDeepestSuggestionNode(module, matchesRootTillParentNode, pathSegments,
        ...
        return doWithDelegateAndReturn(delegate -> {
            ...
            if (!isEmpty(matches)) {
                return matches.stream().map(helper -> {
                    // TODO: Need to identify a better mechanism than this dirty way. Probably use
                    //  ClassSuggestionNode as return type for findDirectChildrenForQueryPrefix
                    // since we are in an iterable(multiple values), keys would be requested, only if the object we
                    // are referring is not a leaf => GenericClassWrapper
    ...
    protected SortedSet<Suggestion> doFindValueSuggestionsForPrefix(Module module, FileType fileType,
                                                                    ...
                                                                    String prefix,
                                                                    @Nullable Set<String> siblingsToExclude) {
        return doWithDelegateAndReturn(delegate -> delegate.findValueSuggestionsForPrefix(module, fileType,
                matchesRootTillMe, prefix, siblingsToExclude), null);
    }
    ...
    protected String doGetDocumentationForValue(Module module, String nodeNavigationPathDotDelimited,
        ...
        return doWithDelegateAndReturn(delegate -> delegate.getDocumentationForValue(module,
                nodeNavigationPathDotDelimited, originalValue), null);
    }
    ...
    private <T> T doWithDelegateOrReturnNull(MetadataProxyInvokerWithReturnValue<T> targetInvokerWithReturnValue) {
        return doWithDelegateAndReturn(targetInvokerWithReturnValue, null);
    }

    private <T> T doWithDelegateAndReturn(MetadataProxyInvokerWithReturnValue<T> targetInvokerWithReturnValue, T defaultReturnValue) {
        if (delegate != null) {
