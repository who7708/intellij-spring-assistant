...
public class PerGroupDependencyTableModel extends AbstractTableModel implements VersionUpdateListener,
        DependencySelectionChangeListener, DependencyRemovalListener {
    ...
    PerGroupDependencyTableModel(JBTable perGroupDependencyTable, @NotNull ProjectCreationRequest request,
                                 @NotNull DependencyGroup dependencyGroup, @NotNull Version bootVersion,
        ...
        dependencyColumn.setCellRenderer(new ColoredTableCellRenderer() {
            ...
            protected void customizeCellRenderer(JTable table, @Nullable Object value, boolean selected,
                ...
                // Enable search highlighting. This in conjunction with TableSpeedSearch(below) enables type to
                // search capability of intellij
        ...
        perGroupDependencyTable.getInputMap().put(getKeyStroke(VK_SPACE, 0), "select_deselect_dependency");
        ...
        perGroupDependencyTable.getSelectionModel().addListSelectionListener(e -> {
            if (!e.getValueIsAdjusting()) {
                ...
                if (selectedRow != -1) {
                    selectionListeners.forEach(listener -> listener.onDependencySelected(getDependencyAt(selectedRow)));
                } else {
    ...
    public boolean isCellEditable(int rowIndex, int columnIndex) {
        return columnIndex == CHECKBOX_COL_INDEX && (incompatibleDependencyIndexes == null || !incompatibleDependencyIndexes.contains(rowIndex));
    }
    ...
    public void onDependencySelected(@Nullable Dependency dependency) {
        if (dependency != null) {
            ...
            perGroupDependencyTable.getSelectionModel().setSelectionInterval(dependencyIndex, dependencyIndex);
        }
    }
