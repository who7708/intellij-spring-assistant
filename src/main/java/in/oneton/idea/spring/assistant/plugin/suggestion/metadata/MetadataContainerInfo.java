...
public class MetadataContainerInfo {
    ...
    public static final String ADDITIONAL_SPRING_CONFIGURATION_METADATA_JSON = "additional-spring-configuration" +
            "-metadata.json";
    /**
    ...
    public static Stream<String> getContainerArchiveOrFileRefs(VirtualFile fileContainer) {
        if (fileContainer.getFileType() == ARCHIVE) {
            ...
            VirtualFile metadataFile = findMetadataFile(fileContainer, SPRING_CONFIGURATION_METADATA_JSON);
            VirtualFile additionalMetadataFile = findMetadataFile(fileContainer,
    ...
    public static Collection<MetadataContainerInfo> newInstances(VirtualFile fileContainer) {
        ...
        MetadataContainerInfo containerInfo = newInstance(fileContainer, containerFile,
                SPRING_CONFIGURATION_METADATA_JSON, archive);
        containerInfos.add(containerInfo);
        if (!archive) {
            // Even after enabling annotation processor support in intellij, for the projects with
            // `spring-boot-configuration-processor` in classpath, intellij is not merging
            // `spring-configuration-metadata.json` & the generated `additional-spring-configuration-metadata.json`.
            // So lets merge these two ourselves if root is not an archive
            MetadataContainerInfo additionalContainerInfo = newInstance(fileContainer, containerFile,
    ...
    private static MetadataContainerInfo newInstance(VirtualFile fileContainer, VirtualFile containerFile,
                                                     String metadataFileName, boolean archive) {
        ...
        if (metadataFile != null) {
            ...
            builder.fileUrl(metadataFile.getUrl()).containerArchiveOrFileRef(archive ? containerFile.getUrl() :
                    metadataFile.getUrl()).marker(archive ? metadataFile.getModificationCount() :
                    metadataFile.getModificationStamp());
        } else {
            builder.containerArchiveOrFileRef(containerFile.getUrl()).marker(containerFile.getModificationCount());
        }
