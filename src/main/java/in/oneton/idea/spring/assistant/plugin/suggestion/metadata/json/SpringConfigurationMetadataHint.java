...
public class SpringConfigurationMetadataHint implements GsonPostProcessable {
    ...
    public void doOnGsonDeserialization() {
        if (hasPredefinedValues()) {
            ...
            for (SpringConfigurationMetadataHintValue value : requireNonNull(values)) {
                // The default value can be array (if property is of type array) as per documentation, we dont
                // support those usecases as of now
    ...
    public Collection<SpringConfigurationMetadataHintValue> findHintValuesWithPrefix(String querySegmentPrefix) {
        if (valueTrie != null) {
