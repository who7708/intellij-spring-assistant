...
public class ProjectCreationRequest {
    ...
    public boolean hasCompatibleJavaVersion(ModuleBuilder moduleBuilder, WizardContext wizardContext) {
        ...
        if (wizardSdkVersion != null) {
            ...
            JavaSdkVersion selectedSdkVersion = selectedLanguageLevel != null ?
                    fromLanguageLevel(selectedLanguageLevel) : null;
    ...
    public String buildDownloadUrl() {
        //@formatter:off
        return serverUrl + "/" + type.getAction() + "?" + nameAndValueAsUrlParam("type", type.getId()) + "&" + nameAndValueAsUrlParam("groupId", groupId) + "&" + nameAndValueAsUrlParam("artifactId", artifactId) + "&" + nameAndValueAsUrlParam("version", version) + "&" + nameAndValueAsUrlParam("name", name) + "&" + nameAndValueAsUrlParam("description", description) + "&" + nameAndValueAsUrlParam("packageName", packageName) + "&" + nameAndValueAsUrlParam("language", language.getId()) + "&" + nameAndValueAsUrlParam("javaVersion", javaVersion.getId()) + "&" + nameAndValueAsUrlParam("packaging", packaging.getId()) + "&" + nameAndValueAsUrlParam("bootVersion", bootVersion.toString()) + "&" + dependencies.stream().map(Dependency::getId).map(dependencyId -> nameAndValueAsUrlParam("dependencies", dependencyId)).collect(joining("&"));
        //@formatter:on
    }

    public <T> T getSetProperty(@NotNull Consumer<T> setter, @NotNull Supplier<T> getter, @Nullable T defaultValue) {
        if (getter.get() == null) {
    ...
    @Nullable
    public <T extends IdContainer> T getSetIdContainer(@NotNull Consumer<T> setter, @NotNull Supplier<T> getter,
                                                       @NotNull Collection<T> containers, @Nullable String defaultId) {
        if (getter.get() == null && defaultId != null) {
            containers.stream().filter(packagingType -> packagingType.getId().equals(defaultId)).findFirst().ifPresent(setter);
        }
        return getter.get();
    }

    @Nullable
    public Version getSetVersion(@NotNull Collection<IdAndName> containers, @Nullable String defaultVersionId) {
        if (bootVersion == null && defaultVersionId != null) {
            Version defaultVersion = Version.parse(defaultVersionId);
            containers.stream().filter(idAndName -> idAndName.parseIdAsVersion().equals(defaultVersion)).findFirst().ifPresent(v -> bootVersion = v.parseIdAsVersion());
        }
        return bootVersion;
    }
