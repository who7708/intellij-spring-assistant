...
public class ProjectDetails {
    ...
    public void init(InitializrModuleBuilder moduleBuilder, WizardContext wizardContext) {
        ...
        IdAndName defaultPackagingType = request.getSetIdContainer(request::setPackaging, request::getPackaging,
                packagingTypes, packagingTypeComposite.getDefaultValue());
        String defaultPackagingTypeId = defaultPackagingType != null ? defaultPackagingType.getId() : null;
        CollectionComboBoxModel<IdAndName> packagingTypeComboBoxModel = newCollectionComboBoxModel(packagingTypes,
        ...
        CollectionComboBoxModel<IdAndName> javaVersionComboBoxModel = newCollectionComboBoxModel(javaVersions,
                defaultJavaVersionId);
        ...
        IdAndName defaultLanguage = request.getSetIdContainer(request::setLanguage, request::getLanguage, languages,
                languageComposite.getDefaultValue());
        String defaultLanguageId = defaultLanguage != null ? defaultLanguage.getId() : null;
        CollectionComboBoxModel<IdAndName> projectLanguageComboBoxModel = newCollectionComboBoxModel(languages,
                defaultLanguageId);
        ...
        if (project != null && project.isInitialized()) {
            boolean rootProjectIsGradle = findFileUnderRootInModule(wizardContext.getProject().getBaseDir(), "build" +
                    ".gradle") != null;
            if (rootProjectIsGradle) {
                defaultProjectTypeId = "gradle-project";
            } else {
                boolean rootProjectIsMaven = findFileUnderRootInModule(wizardContext.getProject().getBaseDir(), "pom" +
                        ".xml") != null;
        ...
        ProjectType defaultProjectType = request.getSetIdContainer(request::setType, request::getType, projectTypes,
                defaultProjectTypeId);
        defaultProjectTypeId = defaultProjectType != null ? defaultProjectType.getId() : null;
        CollectionComboBoxModel<ProjectType> projectTypeComboBoxModel = newCollectionComboBoxModel(projectTypes,
                defaultProjectTypeId);
    ...
    public boolean validate(ModuleBuilder moduleBuilder, WizardContext wizardContext) throws ConfigurationException {
        if (!request.hasValidGroupId()) {
            ...
            throw new ConfigurationException("Selected Java version " + requireNonNull((IdAndName) javaVersion.getSelectedItem()).getName() + " is not supported. Max supported version is (" + requireNonNull(wizardSdkVersion).getMaxLanguageLevel().getCompilerComplianceDefaultOption() + ").\n\n You can go back to first screen and change the Project/Module SDK version there if you need support for newer Java versions", "Java Compatibility");
        }
        return true;
    }
