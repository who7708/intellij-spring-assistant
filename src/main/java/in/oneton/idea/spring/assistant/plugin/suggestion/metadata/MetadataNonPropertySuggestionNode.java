 ...
 * </ul>
 * <p>
 * We can expect a total of 5 tries in the complete search tree, each child trie being hosted by the parent trie
 * element (+ a special toplevel trie for the whole tree)
 * <ul>
...
public class MetadataNonPropertySuggestionNode extends MetadataSuggestionNode {
    ...
    public static MetadataNonPropertySuggestionNode newInstance(String originalName,
                                                                @Nullable MetadataNonPropertySuggestionNode parent,
                                                                String belongsTo) {
        MetadataNonPropertySuggestionNodeBuilder builder =
                MetadataNonPropertySuggestionNode.builder().name(SuggestionNode.sanitise(originalName)).originalName(originalName).parent(parent);
    ...
    @Override
    public MetadataSuggestionNode findDeepestMetadataNode(String[] pathSegments, int pathSegmentStartIndex,
            ...
            if (hasChildren()) {
                ...
                if (childLookup.containsKey(pathSegment)) {
                    ...
                    if (lastSegment) {
                        ...
                        deepestMatch = child.findDeepestMetadataNode(pathSegments, pathSegmentStartIndex + 1,
                                matchAllSegments);
                    }
    ...
    @Nullable
    @Override
    public List<SuggestionNode> findDeepestSuggestionNode(Module module, List<SuggestionNode> matchesRootTillMe,
    ...
    public void addChildren(Module module, SpringConfigurationMetadataGroup group, String[] rawPathSegments,
                            int startIndex, String belongsTo) {
        MetadataNonPropertySuggestionNode groupNode = addChildren(rawPathSegments, startIndex,
                rawPathSegments.length - 1, belongsTo);
        groupNode.setGroup(module, group);
    }

    public void addChildren(SpringConfigurationMetadataProperty property, String[] rawPathSegments, int startIndex,
                            String belongsTo) {
    ...
    public SortedSet<Suggestion> findKeySuggestionsForQueryPrefix(Module module, FileType fileType,
                                                                  ...
                                                                  int numOfAncestors, String[] querySegmentPrefixes,
        ...
        if (lookingForConcreteNode) {
            if (isGroup()) {
                // If we have only one child, lets send the child value directly instead of this node. This way user
                // does not need trigger suggestion for level, esp. when we know there will is only be one child
                if (hasOnlyOneChild(module)) {
                    ...
                    return addChildToMatchesAndSearchInNextLevel(module, fileType, matchesRootTillMe, numOfAncestors,
                            querySegmentPrefixes, querySegmentPrefixStartIndex, childrenTrie.values());
                } else { // either there are no children/multiple children are present. Lets return suggestions
                    assert group != null;
                    return newSingleElementSortedSet(group.newSuggestion(fileType, matchesRootTillMe, numOfAncestors));
                }
            ...
            if (hasChildren()) {
                ...
                if (!isEmpty(exclusionMembers) && !isEmpty(matchedChildren)) {
                    ...
                    matchedChildren =
                            matchedChildren.stream().filter(value -> !finalExclusionMembers.contains(value)).collect(toList());
                }

                if (matchedChildren.size() != 0) {
                    SortedSet<Suggestion> suggestions = addChildToMatchesAndSearchInNextLevel(module, fileType,
                            matchesRootTillMe, numOfAncestors, querySegmentPrefixes, querySegmentPrefixStartIndex + 1
                            , matchedChildren);
                    // If the leaf is deprecated (management.context-path), it will shadow deeper match (management
    ...
    public String toTree() {
        StringBuilder builder = new StringBuilder(originalName).append(isRoot() ? "(root + group)" : (isGroup() ?
                "(group)" : "(intermediate)")).append("\n");
        if (childLookup != null) {
            childLookup.forEach((k, v) -> builder.append(v.toTree().trim().replaceAll("^", "  ").replaceAll("\n", "\n" +
                    "  ")).append("\n"));
        }
    ...
    public String getDocumentationForKey(Module module, String nodeNavigationPathDotDelimited) {
        ...
        throw new RuntimeException("Documentation not supported for this element. Call supportsDocumentation() first");
    }
    ...
    @Nullable
    @Override
    public String getDocumentationForValue(Module module, String nodeNavigationPathDotDelimited, String originalValue) {
        throw new IllegalAccessError("Should never be called");
    }
    ...
    private Collection<MetadataSuggestionNode> computeChildrenToIterateOver(@NotNull Map<String,
            MetadataSuggestionNode> childLookup, Set<MetadataSuggestionNode> exclusionMembers) {
        Collection<MetadataSuggestionNode> childrenToIterateOver;
        if (!isEmpty(exclusionMembers)) {
            childrenToIterateOver =
                    childLookup.values().stream().filter(value -> !exclusionMembers.contains(value)).collect(toList());
        } else {
    ...
    private void addProperty(SpringConfigurationMetadataProperty property, String originalName, String belongsTo) {
        addRefCascadeTillRoot(belongsTo);
        if (!hasChildren()) {
        ...
        assert childLookup != null;
        assert childrenTrie != null;
        MetadataSuggestionNode childNode = MetadataPropertySuggestionNode.newInstance(originalName, property, this,
                belongsTo);
    ...
    private MetadataNonPropertySuggestionNode addChildren(String[] rawPathSegments, int startIndex, int endIndexIncl,
                                                          String belongsTo) {
        addRefCascadeTillRoot(belongsTo);
        if (!hasChildren()) {
            childLookup = new THashMap<>();
        ...
        MetadataNonPropertySuggestionNode childNode = (MetadataNonPropertySuggestionNode) childLookup.get(pathSegment);
        if (childNode == null) {
    ...
    private SortedSet<Suggestion> addChildToMatchesAndSearchInNextLevel(Module module, FileType fileType,
                                                                        List<SuggestionNode> matchesRootTillParentNode, int numOfAncestors, String[] querySegmentPrefixes, int querySegmentPrefixStartIndex, Collection<MetadataSuggestionNode> childNodes) {
        SortedSet<Suggestion> suggestions = null;
        for (MetadataSuggestionNode child : childNodes) {
            List<SuggestionNode> matchesRootTillChild = unmodifiableList(newListWithMembers(matchesRootTillParentNode, child));
            Set<Suggestion> matchedSuggestions = child.findKeySuggestionsForQueryPrefix(module, fileType, matchesRootTillChild, numOfAncestors, querySegmentPrefixes, querySegmentPrefixStartIndex, null);
