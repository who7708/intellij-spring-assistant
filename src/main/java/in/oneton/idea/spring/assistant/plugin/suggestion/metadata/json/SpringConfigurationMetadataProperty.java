...
/**
 * Refer to https://docs.spring.io/spring-boot/docs/2.0.0
 * .M6/reference/htmlsingle/#configuration-metadata-property-attributes
 */
@EqualsAndHashCode(of = "name")
public class SpringConfigurationMetadataProperty implements Comparable<SpringConfigurationMetadataProperty> {
    ...
    public List<SuggestionNode> findChildDeepestKeyMatch(Module module,
                                                         List<SuggestionNode> matchesRootTillParentNode,
        ...
        if (!isLeaf(module)) {
            if (isMapWithPredefinedKeys()) { // map
                ...
                SpringConfigurationMetadataHintValue valueHint = genericOrKeyHint.findHintValueWithName(pathSegment);
                if (valueHint != null) {
                    ...
                    if (lastPathSegment) {
                        ...
                        if (!isMapWithPredefinedValues()) {
                            return doWithDelegateOrReturnNull(module,
                                    delegate -> delegate.findDeepestSuggestionNode(module, matchesRootTillParentNode,
                                            pathSegments, pathSegmentStartIndex));
                        }
                    }
                }
            } else {
                return doWithDelegateOrReturnNull(module, delegate -> delegate.findDeepestSuggestionNode(module,
                        matchesRootTillParentNode, pathSegments, pathSegmentStartIndex));
            }
        }
        return null;
    }
    ...
    @Nullable
    public SortedSet<Suggestion> findChildKeySuggestionsForQueryPrefix(Module module, FileType fileType,
        ...
        if (lastPathSegment && !isLeaf(module)) {
            if (isMapWithPredefinedKeys()) { // map
                ...
                return matchesStream.map(hintValue -> {
                    ...
                    return hintValue.buildSuggestionForKey(fileType, matchesRootTillMe, numOfAncestors,
                            suggestionNode, getMapKeyType(module));
                }).collect(toCollection(TreeSet::new));
            } else {
                return doWithDelegateOrReturnNull(module,
                        delegate -> delegate.findKeySuggestionsForQueryPrefix(module, fileType, matchesRootTillMe,
    ...
    @NotNull
    public Suggestion buildKeySuggestion(Module module, FileType fileType, List<SuggestionNode> matchesRootTillMe,
                                         int numOfAncestors) {
        Suggestion.SuggestionBuilder builder =
                Suggestion.builder().suggestionToDisplay(GenericUtil.dotDelimitedOriginalNames(matchesRootTillMe,
                        numOfAncestors)).description(description).shortType(shortenedType(className)).defaultValue(getDefaultValueAsStr()).numOfAncestors(numOfAncestors).matchesTopFirst(matchesRootTillMe).icon(getSuggestionNodeType(module).getIcon());
        if (deprecation != null) {
            builder.deprecationLevel(deprecation.getLevel() != null ? deprecation.getLevel() :
    ...
    public String getDocumentationForKey(String nodeNavigationPathDotDelimited) {
         ...
         */
        StringBuilder builder = new StringBuilder().append("<b>").append(nodeNavigationPathDotDelimited).append("</b>");
        ...
        if (deprecation != null) {
            builder.append("<p><b>").append(isDeprecatedError() ? "ERROR: DO NOT USE THIS PROPERTY AS IT IS " +
                    "COMPLETELY UNSUPPORTED" : "WARNING: PROPERTY IS DEPRECATED").append("</b></p>");
    ...
    public boolean isLeaf(Module module) {
        return isLeafWithKnownValues() || getSuggestionNodeType(module).representsLeaf() || doWithDelegateOrReturnDefault(module, delegate -> delegate.isLeaf(module), true);
    }
    ...
    public boolean isDeprecatedError() {
        return deprecation != null && deprecation.getLevel() == SpringConfigurationMetadataDeprecationLevel.error;
    }

    public SortedSet<Suggestion> findSuggestionsForValues(Module module, FileType fileType,
                                                          List<SuggestionNode> matchesRootTillContainerProperty,
                                                          String prefix, @Nullable Set<String> siblingsToExclude) {
        ...
        if (nodeType == VALUES) {
            ...
            if (!isEmpty(matches)) {
                ...
                return matchesStream.map(match -> match.buildSuggestionForValue(fileType,
                        matchesRootTillContainerProperty, getDefaultValueAsStr(), getPsiType(module))).collect(toCollection(TreeSet::new));
            }
        } else {
            return doWithDelegateOrReturnNull(module, delegate -> delegate.findValueSuggestionsForPrefix(module,
                    fileType, matchesRootTillContainerProperty, prefix, siblingsToExclude));
        }
    ...
    private Stream<SpringConfigurationMetadataHintValue> getMatchesAfterExcludingSiblings(@NotNull SpringConfigurationMetadataHint hintFindValueAgainst, Collection<SpringConfigurationMetadataHintValue> matches, @Nullable Set<String> siblingsToExclude) {
        Stream<SpringConfigurationMetadataHintValue> matchesStream;
        if (siblingsToExclude != null) {
            Set<SpringConfigurationMetadataHintValue> exclusionMembers =
                    siblingsToExclude.stream().map(hintFindValueAgainst::findHintValueWithName).collect(toSet());
    ...
    private boolean isLeafWithKnownValues() {
        return !isMapWithPredefinedKeys() && !isMapWithPredefinedValues() && genericOrKeyHint != null && genericOrKeyHint.hasPredefinedValues();
    }
    ...
    private <T> T doWithMapDelegateOrReturnNull(Module module, MetadataProxyInvokerWithReturnValue<T> invoker) {
        MetadataProxy delegate = getDelegate(module);
        if (delegate != null) {
    ...
    private String getDefaultValueAsStr() {
        if (defaultValue != null && !(defaultValue instanceof Array) && !(defaultValue instanceof Collection)) {
            if (className != null && defaultValue instanceof Double) {
                // if defaultValue is a number, its being parsed by gson as double & we will see an incorrect
                // fraction when we take toString()
                switch (className) {
    ...
    public String getDocumentationForValue(Module module, String nodeNavigationPathDotDelimited, String value) {
        if (isLeafWithKnownValues()) {
            assert genericOrKeyHint != null;
            SpringConfigurationMetadataHintValue hintValueWithName = genericOrKeyHint.findHintValueWithName(value);
            if (hintValueWithName != null) {
                return hintValueWithName.getDocumentationForValue(nodeNavigationPathDotDelimited,
                        getMapValueType(module));
            }
        } else {
            // possible this represents an enum
            return doWithDelegateOrReturnNull(module, delegate -> delegate.getDocumentationForValue(module,
                    nodeNavigationPathDotDelimited, value));
        }
        return null;
    }
    ...
    class HintAwareSuggestionNode implements SuggestionNode {
        ...
        public SortedSet<Suggestion> findKeySuggestionsForQueryPrefix(Module module, FileType fileType,
            ...
            return doWithMapDelegateOrReturnNull(module,
                    delegate -> ((MapClassMetadataProxy) delegate).findChildKeySuggestionForQueryPrefix(module, fileType, matchesRootTillMe, numOfAncestors, querySegmentPrefixes, querySegmentPrefixStartIndex));
        }
        ...
        public SortedSet<Suggestion> findKeySuggestionsForQueryPrefix(Module module, FileType fileType,
                                                                      ...
                                                                      @Nullable Set<String> siblingsToExclude) {
            return doWithMapDelegateOrReturnNull(module,
                    delegate -> ((MapClassMetadataProxy) delegate).findChildKeySuggestionForQueryPrefix(module, fileType, matchesRootTillMe, numOfAncestors, querySegmentPrefixes, querySegmentPrefixStartIndex, siblingsToExclude));
        }
        ...
        public SortedSet<Suggestion> findValueSuggestionsForPrefix(Module module, FileType fileType,
            ...
            if (isMapWithPredefinedValues()) {
                ...
                Collection<SpringConfigurationMetadataHintValue> matches = valueHint.findHintValuesWithPrefix(prefix);
                if (matches != null && matches.size() != 0) {
                    return matches.stream().map(match -> match.buildSuggestionForValue(fileType, matchesRootTillMe,
                            getDefaultValueAsStr(), getMapValueType(module))).collect(toCollection(TreeSet::new));
                }
            } else {
                return doWithDelegateOrReturnNull(module, delegate -> delegate.findValueSuggestionsForPrefix(module,
                        fileType, matchesRootTillMe, prefix));
            }
            return null;
        }
        ...
        public SortedSet<Suggestion> findValueSuggestionsForPrefix(Module module, FileType fileType,
            ...
            if (isMapWithPredefinedValues()) {
                ...
                if (!isEmpty(matches)) {
                    ...
                    return matchesStream.map(match -> match.buildSuggestionForValue(fileType, matchesRootTillMe,
                            getDefaultValueAsStr(), getMapValueType(module))).collect(toCollection(TreeSet::new));
                }
            } else {
                return doWithDelegateOrReturnNull(module, delegate -> delegate.findValueSuggestionsForPrefix(module,
        ...
        public String getDocumentationForValue(Module module, String nodeNavigationPathDotDelimited,
            ...
            if (isMapWithPredefinedValues()) {
                ...
                return hint.getDocumentationForValue(nodeNavigationPathDotDelimited, getMapValueType(module));
            } else {
                return doWithDelegateOrReturnNull(module, delegate -> delegate.getDocumentationForValue(module, nodeNavigationPathDotDelimited, originalValue));
            }
        }
