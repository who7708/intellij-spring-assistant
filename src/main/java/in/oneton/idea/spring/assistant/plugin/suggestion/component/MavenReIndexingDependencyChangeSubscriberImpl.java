...
public class MavenReIndexingDependencyChangeSubscriberImpl implements MavenReIndexingDependencyChangeSubscriber,
        ProjectComponent {
    ...
    public void projectOpened() {
        ...
        try {
            ...
            connection.subscribe(MavenImportListener.TOPIC, (importedProjects, newModules) -> {
                boolean proceed =
                        importedProjects.stream().anyMatch(project -> this.project.getName().equals(project.getName()) && project.getDirectory().equals(this.project.getBasePath()));
                    ...
                    DumbService.getInstance(project).smartInvokeLater(() -> {
                        ...
                        try {
                            ...
                            if (modules.length > 0) {
                                ...
                                debug(() -> log.debug("Skipping indexing for project " + project.getName() + " as " +
                                        "there are no modules"));
                            }
                        } catch (Throwable e) {
                            log.error("Error occurred while indexing project " + project.getName() + " & modules " + moduleNamesAsStrCommaDelimited(newModules, false), e);
                        }
                    });
                } else {
