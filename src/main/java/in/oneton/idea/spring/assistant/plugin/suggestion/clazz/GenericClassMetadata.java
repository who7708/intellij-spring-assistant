...
public class GenericClassMetadata extends ClassMetadata {
    ...
    protected SuggestionDocumentationHelper doFindDirectChild(Module module, String pathSegment) {
        // TODO: Does spring Environment support setting any type other than boolean, number, string & enum to be set
        //  as keys. If not we should throw an exception
        // For now lets allow
        ...
        if (!isLeaf(module)) {
            ...
            if (childLookup != null && childLookup.containsKey(pathSegment)) {
                ...
                if (lastPathSegment) {
                    ...
                    return memberWrapper.getMemberReferredClassMetadataProxy(module).findDeepestSuggestionNode(module
        ...
        if (!isLeaf(module)) {
            if (childrenTrie != null && childLookup != null) {
                ...
                if (!isEmpty(sortedPrefixToMemberWrapper)) {
                    Collection<GenericClassMemberWrapper> wrappers = getMatchesAfterExclusions(childLookup,
                            sortedPrefixToMemberWrapper, siblingsToExclude);
                    if (!isEmpty(wrappers)) {
                        ...
                        if (lastQuerySegment) {
                            ...
                            for (GenericClassMemberWrapper wrapper : wrappers) {
                                ...
                                Set<Suggestion> matchedSuggestions =
                                        wrapper.getMemberReferredClassMetadataProxy(module).findKeySuggestionsForQueryPrefix(module, fileType, pathRootTillCurrentNode, numOfAncestors, querySegmentPrefixes, querySegmentPrefixStartIndex);
    ...
    private Collection<GenericClassMemberWrapper> getMatchesAfterExclusions(@NotNull Map<String,
            GenericClassMemberWrapper> childLookup, SortedMap<String, GenericClassMemberWrapper> sortedPrefixToMemberWrapper, @Nullable Set<String> siblingsToExclude) {
        ...
        if (siblingsToExclude != null) {
            ...
            wrappers = wrappers.stream().filter(value -> !exclusionMembers.contains(value)).collect(toList());
        }
        return wrappers;
    }
