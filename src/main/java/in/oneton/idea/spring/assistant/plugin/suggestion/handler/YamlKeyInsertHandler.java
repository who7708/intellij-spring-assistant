...
public class YamlKeyInsertHandler implements InsertHandler<LookupElement> {
    ...
    public void handleInsert(final InsertionContext context, final LookupElement lookupElement) {
        if (!nextCharAfterSpacesAndQuotesIsColon(getStringAfterAutoCompletedValue(context))) {
            ...
            String suggestionWithCaret = getSuggestionReplacementWithCaret(module, suggestion, existingIndentation,
                    indentPerLevel);
    ...
    private void deleteLookupTextAndRetrieveOldValue(InsertionContext context, @NotNull PsiElement elementAtCaret) {
        if (elementAtCaret.getNode().getElementType() != YAMLTokenTypes.SCALAR_KEY) {
            ...
            if (keyValue.getValue() != null) {
                YAMLKeyValue dummyKV = YAMLElementGenerator.getInstance(context.getProject()).createYamlKeyValue("foo"
                        , "b");
                dummyKV.setValue(keyValue.getValue());
            }

            context.setTailOffset(keyValue.getTextRange().getEndOffset());
            runWriteCommandAction(context.getProject(), () -> keyValue.getParentMapping().deleteKeyValue(keyValue));
        }
    }
    ...
    @NotNull
    private String getSuggestionReplacementWithCaret(Module module, Suggestion suggestion, String existingIndentation
        ...
        do {
            ...
            builder.append("\n").append(existingIndentation).append(getIndent(indentPerLevel, i)).append(nameProvider.getOriginalName()).append(":");
            i++;
        } while (i < matchesTopFirst.size());
        builder.delete(0, existingIndentation.length() + 1);
        String indentForNextLevel = getOverallIndent(existingIndentation, indentPerLevel, matchesTopFirst.size());
    ...
    @NotNull
    private String getPlaceholderSufixWithCaret(Module module, Suggestion suggestion, String indentForNextLevel) {
