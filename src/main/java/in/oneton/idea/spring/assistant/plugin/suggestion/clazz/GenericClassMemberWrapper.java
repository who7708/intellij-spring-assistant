 ...
 * This acts as a proxy for documentation & basic details about the property/getter method that contains actual
 * documentation
 */
public class GenericClassMemberWrapper implements SuggestionNode, SuggestionDocumentationHelper {
    ...
    private <T> T doWithMemberReferredClassProxy(Module module, ProxyInvoker<T> invoker, T defaultValue) {
    ...
    public List<SuggestionNode> findDeepestSuggestionNode(Module module,
        ...
        return doWithMemberReferredClassProxy(module, proxy -> proxy.findDeepestSuggestionNode(module,
                matchesRootTillParentNode, pathSegments, pathSegmentStartIndex), null);
    }
    ...
    public SortedSet<Suggestion> findKeySuggestionsForQueryPrefix(Module module, FileType fileType,
        ...
        return doWithMemberReferredClassProxy(module, proxy -> proxy.findKeySuggestionsForQueryPrefix(module,
                fileType, matchesRootTillMe, numOfAncestors, querySegmentPrefixes, querySegmentPrefixStartIndex), null);
    }
    ...
    public SortedSet<Suggestion> findKeySuggestionsForQueryPrefix(Module module, FileType fileType,
        ...
        return doWithMemberReferredClassProxy(module, proxy -> proxy.findKeySuggestionsForQueryPrefix(module,
                fileType, matchesRootTillMe, numOfAncestors, querySegmentPrefixes, querySegmentPrefixStartIndex,
    ...
    public SortedSet<Suggestion> findValueSuggestionsForPrefix(Module module, FileType fileType,
                                                               List<SuggestionNode> matchesRootTillMe, String prefix,
                                                               @Nullable Set<String> siblingsToExclude) {
        return doWithMemberReferredClassProxy(module, proxy -> proxy.findValueSuggestionsForPrefix(module, fileType,
    ...
    private boolean computeDeprecationStatus() {
        if (member instanceof PsiField) {
            ...
            return stream(requireNonNull(((PsiMethod) member).getReturnType()).getAnnotations()).anyMatch(annotation -> {
                String fqn = annotation.getQualifiedName();
                return fqn != null && fqn.equals("java.lang.Deprecated");
            });
        }
    ...
    @NotNull
    @Override
    public Suggestion buildSuggestionForKey(Module module, FileType fileType, List<SuggestionNode> matchesRootTillMe,
                                            int numOfAncestors) {
        ...
        Suggestion.SuggestionBuilder builder =
                Suggestion.builder().suggestionToDisplay(originalName).description(documentation).shortType(shortType).numOfAncestors(numOfAncestors).matchesTopFirst(matchesRootTillMe).icon(icon);
        if (deprecated) {
    ...
    public String getDocumentationForKey(Module module, String nodeNavigationPathDotDelimited) {
        return "<b>" + nodeNavigationPathDotDelimited + "</b>" + new JavaDocumentationProvider().generateDoc(member,
                member);
    }

    @Override
    public String getDocumentationForValue(Module module, String nodeNavigationPathDotDelimited, String originalValue) {
        return "<b>" + nodeNavigationPathDotDelimited + "</b> =  <b>" + unescapeValue(originalValue) + "</b>" + new JavaDocumentationProvider().generateDoc(member, member);
    }

    @NotNull
    @Override
    public SuggestionNodeType getSuggestionNodeType(Module module) {
        return doWithMemberReferredClassProxy(module, proxy -> proxy.getSuggestionNodeType(module), SuggestionNodeType.UNKNOWN_CLASS);
    }
